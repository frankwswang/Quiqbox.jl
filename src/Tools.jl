...
function splitFloatNumber(x::Float64)
    str = string(x)
    s = x < 0 ? -1 : 1
    l = floor(x |> abs) |> Int
    r = parse(Int, split(str, ".")[2])
    Int[s, l, r]
end

mergeFloatNumber(s::Int, ldp::Int, rdp::Int) = 
s * parse(Float64, string(ldp) * "." * string(rdp))

# struct Pf{S, LDP, RDP, F} <: ParameterizedFunction
#     c::Float64
#     f::Function

#     function Pf(c::Float64, f::Function)
#         S, LDP, RDP = splitFloatNumber(c)
#         new{S, LDP, RDP, nameOf(f)}(c, f)
#     end

#     function Pf(c::Float64, f::Pf{<:Any, <:Any, <:Any, F}) where {F}
#         cNew = c*f.c
#         S, LDP, RDP = splitFloatNumber(cNew)
#         new{S, LDP, RDP, F}(cNew, f.f)
#     end
# end
...
# (::Type{Pf{S, LDP, RDP, F}})(x) where {S, LDP, RDP, F} = 
# mergeFloatNumber(S, LDP, RDP) * getFunc(F, NaN)(x)


# PfDict = Dict{DataType, Function}(Pf{1.0, :itself}=>itself)

# function getPf!(c::Real, f::Function, dict=PfDict)
#     coeff = convert(Float64, c)::Float64
#     val = get(dict, Pf{coeff, nameOf(f)}, false)
#     if val == false
#         Pf(coeff, f)
#     else
#         val
...
# function getPf(c::Real, f::Function)
#     # Pf(c, f)
#     Pf(c, f)
#     # itself
# end
...
#     Pf(c::Float64, f::Function) = new{c, typeof(f)}(f)
#     Pf(c::Float64, f::Pf{C, F}) where {C, F} = new{c*C, F}(f.f)
#     Pf(c::Float64, f::typeof(itself)) = new{c, typeof(itself)}(f)
#     Pf(c::Float64, f::Pf{C, typeof(itself)}) where {C} = new{c*C, typeof(itself)}(f.f)
# end


# # (f::Pf{C})(x) where {C} = C * f.f(x)
# (f::Pf{C})(x) where {C} = C * f.f(x)
# (f::Pf{C, typeof(itself)})(x) where {C} = C * x
# # (f::Pf{C, F})(c::Real, f2::Pf{C, F}) where {C, F} = Pf(c, f2)
# (::Type{Pf{C, F}})(x) where {C, F} = C * (F.instance)(x)
# (::Type{Pf{C, typeof(itself)}})(x) where {C} = C * x
...
# Pf(c::Float64, f::Function) = Pf{c, nameOf(f)}(c, f)
# Pf(c::Float64, f::Pf{<:Any, F}) where {F} = Pf{c*f.c, F}(c*f.c, f.f)
# Pf(c::Float64, ::Val{T}) where {T} = Pf{c, T}(c, getFunc(T, NaN))
# Pf(c::Float64, ::Val{Pf{C, F}}) where {C, F} = Pf{c*C, F}(c*C, getFunc(F, NaN))


# (f::Pf{C})(x::Real) where {C} = C * f.f(x)
# (::Type{Pf{C, F}})(x::Real) where {C, F} = C * getFunc(F, NaN)(x)

# Pf(c::Float64, f::Pf{<:Any, :itself}) = Pf{c*f.c, :itself}(c*f.c, itself)
# Pf(c::Float64, ::Val{:itself}) = Pf{c, :itself}(c, itself)
# Pf(c::Float64, ::Val{Pf{C, :itself}}) where {C} = Pf{c*C, :itself}(c*C, itself)
...
# mappingArray = Function[itself]

# struct Pf{C, FN} <: ParameterizedFunction
#     c::Float64
#     f::Function
#     # Pf(c::Float64, ::typeof(itself)) = new{c, 1}(itself)
#     # Pf(c::Float64, ::Pf{C, 1}) where {C} = new{c*C, 1}(itself)
# end

# Pf(c::Float64, f::Function) = Pf{c, getMappingIndex!(f)}(c, f)
# Pf(c::Float64, f::Pf{C, FN}) where {C, FN} = Pf{c*C, FN}(c, f.f)

# function getMappingIndex!(f::F, arr=mappingArray)::Int where {F<:Function}
#     idx = findfirst(x->typeof(x)===F, arr)
#     if idx === nothing
#         push!(arr, f)
#         lastindex(arr)
#     else
...
# # (f::Pf{C})(x) where {C} = C * f.f(x)
# (f::Pf{C})(x) where {C} = C * f.f(x)
# # (f::Pf{C, 1})(x) where {C} = C * x
# # (f::Pf{C, F})(c::Real, f2::Pf{C, F}) where {C, F} = Pf(c, f2)
# (::Type{Pf{C, FN}})(x) where {C, FN} = C * mappingArray[FN](x)
# # (::Type{Pf{C, 1}})(x) where {C} = C * x
