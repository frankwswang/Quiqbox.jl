...
function prepareOrbitalInfoCore(field::FloatingPolyGaussField{T, D}) where {T<:Real, D}
    ...
    pgf = last(getFieldFuncCorePair(grf).first.encode)          #> `GaussFieldFunc`
    xpnFormatter = last(getFieldFuncCorePair(pgf).first.encode) #> `ParamFormatter`
    xpn = xpnFormatter.core(field.param).xpn
    angMomField = last(gtf.encode)                              #> `CartAngMomentumFunc`
    amfCore = getFieldFuncCorePair(angMomField).first.core.f    #> `CartSHarmonics`
    ang = amfCore.m.tuple
